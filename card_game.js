...
function shuffle(deck) {
  ...
  let currentIndex = deck.length; //starting at 52
  let temporaryValue;
  let randomIndex;
...
function guess(card1, card2) {
  ...
  let input = getInput("Will the next card be higher or lower? (h) or (l)")
  if (input === "h") {
    return compare(card1, card2) < 0;
  } else if (input === "l") {
    return compare(card1, card2) > 0;
  } else {
    console.log("You need to guess h or l, you get no points for this round")
  }
  ...
  function playGame() {
    ...
    while (score < 5 && score < deck.length) {
      ...
      if (guess(currentCard, nextCard) === true) {
        score++;
        console.log(`Congradulations! Your score is ${score}`)
      } else {
        console.log(`Sorry, you were wrong and got no points`)
      }
      currentCard = nextCard;
    }
    deck.length === 0 ? console.log("Yaaayyy! YOU WON!!!") : console.log("You are out of cards, You have lost!");
  }
